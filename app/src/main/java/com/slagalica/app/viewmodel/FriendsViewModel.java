...
public class FriendsViewModel extends ViewModel {
    ...
    public void sendInvite(Friend friend) {
        pendingInviteUsername.setValue(friend.getUsername());
        friendsRepository.sendInvite(friend.getUid(), new RepositoryCallback<String>() {
    ...
    private void listenToOutgoingInvite(String inviteId) {
        ...
        outgoingListener = new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                String status = snapshot.child("status").getValue(String.class);
                if ("accepted".equals(status) || "declined".equals(status) || "expired".equals(status) || "cancelled".equals(status)) {
